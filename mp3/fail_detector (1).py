...
class Fail_Dector_Server:
    def __init__(self,args):
        ...
        self.id = f"{self.host}:{self.port}:{self.timejoin}"
        ...
        self.MembershipList = {
            ...
            for ip, port in [(IP, DEFAULT_PORT_NUM) for IP in [self.host, Introducor]]
        }
        ...
        self.gossipS = False
        self.latest_failed_members = []
        # self.latest_failed_members_flag = False
    ...
    def getMembershipListID(self):
        working_machines = set()
        
        for machine in self.MembershipList.keys():
            if 'fa23-cs425-2910' in machine:
                working_machines.add(10)
            else:
                working_machines.add(int(machine[14]))
    ...
    def removeFailMember(self):
        ...
        with self.rlock:
            ...
            self.latest_failed_members = fail_members_to_remove
            # self.latest_failed_members_flag = True
            # print('latest failed members', self.latest_failed_members)
            # print('failssss, ', fail_members_to_remove)
    ...
    def run_failure_detector(self):
        ...
        :return: None
        """
        receiver_thread = threading.Thread(target=self.receive_fd)
        receiver_thread.daemon = True
        receiver_thread.start()

        # Start a sender thread
        sender_thread = threading.Thread(target=self.send_fd)
        sender_thread.daemon = True
        sender_thread.start()

        # Start a to update enable sending
        user_thread = threading.Thread(target=self.failure_input)
...
if __name__ == "__main__":
    ...
    server = Fail_Dector_Server(args)
    server.run_failure_detector()
