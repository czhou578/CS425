...
class FileServer():
    ...
    def send_packet(self, dest, http_packet, port, request_type = None):
        """
    ...
    def send(self, http_packet, request_type, to_leader, replica_ips=None):
        """
        ...
        if not to_leader:
            if request_type in ['put', 'delete']:
                ...
                for dest in replica_ips:
                    self.send_packet(dest, http_packet, file_reciever_port, request_type)

            elif request_type == 'rereplicate':
                for dest in replica_ips:
                    return self.send_packet(dest, http_packet, file_reciever_port, request_type)
            
            elif request_type in ['get', 'finish_ack']:
                # only need to fetch first one
                self.send_packet(replica_ips[0], http_packet, file_reciever_port, request_type)
        ...
        # send from user to leader
            ...
            print('membership list, ', fail_detector.membership_list.keys())
            print('this is the leader, ', leader_id)
            self.send_packet(leader_id, http_packet, file_leader_port, request_type)
        

    def put_file(self, http_packet):
        """
        ...
        print('inside put file at, ', str(host_domain_name))

        print(f"From {str(source)} to {str(host_domain_name)} for {str(sdfs)}")
        cmd = f'scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {NAME}@{source}:/home/{NAME}/mp4/{local} /home/{NAME}/mp4/MP3_FILE/{sdfs}'
        try:
            result = subprocess.check_output(cmd, shell=True)
            self.logger.info(f"Complete {str(http_packet)} ")
            return_packet = {}
            ...
            self.send(return_packet, 'put_ack', True)
            print('finished in try catch in put_file')

        except Exception as e:
            self.logger.error(f"Command: {cmd}, Error: {str(e)}")


    def get_file(self, http_packet):
        """
        ...
        sdfs = http_packet['sdfs_filename']
        source = http_packet['request_source']
        cmd = f'scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null /home/{NAME}/mp4/MP3_FILE/{sdfs} {NAME}@{source}:/home/{NAME}/mp4/{local}'
        try:
            result = subprocess.check_output(cmd, shell=True)
            self.logger.info(f"Complete {str(http_packet)} ")
            return_packet = {}
    ...
    def update_file(self, http_packet):
        """
        ...
        try:
            if 'payload' in http_packet:
                ...
                self.logger.info(f"File location list update {str(new_file_location)}")
            else:
                del_sdfs = http_packet['sdfs_filename']
                del filelocation_list[del_sdfs]
                self.logger.info(f"Delete {str(del_sdfs)} Success")
        except Exception as e:
            self.logger.error(f'Error {str(e)}')
        

    def handle_request(self, clientsocket, ip):
        """
    ...
    def reciever(self):
        ...
        while True:
            ...
            end_tasks = []
            thread_pool[task_id] = threading.Thread(target=self.handle_request, args=[clientsocket, clientip])
            ...
            for task in end_tasks:
                ...
                self.logger.info("Finish task:[{}]".format(task))

    def send2Member(self):
        """
            ...
            for i in range(len(leader_queue)):
                ...
                if http_packet['request_type'] == 'put': 
                    ...
                    if sdfs_filename in filelocation_list:
                        ...
                        if len(members) >= 4:
                            random_sample = random.sample([x for x in members if x != 'fa23-cs425-2902.cs.illinois.edu'], 2)
                            random_sample.append('fa23-cs425-2902.cs.illinois.edu')
                            random_sample.append(str(host_domain_name))

                            replica_ips = random_sample
                        else:
                            print('len members is < 4')
                    ...
                    self.send(http_packet, 'put', False, replica_ips)
                    ...
                    self.send(location_packet, 'update', False)
                ...
                #READ
                    ...
                    self.send(http_packet, 'get', False, loc)
                    ...
                    try:
                        sdfs_filename = http_packet['sdfs_filename']
                        self.send(http_packet, request_type, False, filelocation_list[sdfs_filename])
                        location_packet['task_id'] = host_domain_name + '_'+str(datetime.datetime.now())  
                        location_packet['request_type'] = 'update'
                        location_packet['sdfs_filename'] = sdfs_filename
                        self.send(location_packet, 'update', False)
                    ...
                    if sorted(list(check_list)) == sorted(put_ack[sdfs_filename]) or len(put_ack[sdfs_filename])>=len(check_list): 
                        ...
                        self.send(ack_packet, 'finish_ack', False, [source])
                        # remove_task.append(i)
                    ...
                    if domain_name in ips:
                        ...
                        # create one new replica
                            ...
                            if dest:
                                ...
                                http_packet['local_filename'] = '/home/{NAME}/mp4/MP3_FILE/' + sdfs_filename
                                ...
                                send_fail = not (self.send(http_packet, 'rereplicate', False, [dest]))
                                if send_fail:
    ...
    def leader_main(self):
        ...
        rereplicate_thread = threading.Thread(target = self.rereplicate)
        rereplicate_thread.start()

        intro_new_join_thread = threading.Thread(target = self.intro_new_join)
        ...
        while True:
            ...
            http_packet = query.decode(msg_format)
            http_packet = json.loads(http_packet)
            self.logger.info(f"Receive {http_packet['request_type']} from {http_packet['request_source']}")
            leader_queue.append(http_packet)
        

    def send2Leader(self, request_type, sdfs_filename, local_filename = None, host_domain_name = host_domain_name):
        """
        ...
        if request_type in ['put', 'get', 'delete']:
            self.send(http_packet, request_type, True)

        else:
            print(f"INVALID request_type {request_type}")

    def clean_local_sdfs_dir(self):
        try:
            # init sdfs
            cmd = 'rm -rf /home/{NAME}/mp4/MP3_FILE'
            result = subprocess.check_output(cmd, shell=True)
            self.logger.info("successfully remove sdfs directory")
            cmd = 'mkdir -p /home/{NAME}/mp4/MP3_FILE'
            result = subprocess.check_output(cmd, shell=True)
            self.logger.info("successfully create sdfs directory")
            # init local to get files from sdfs
            cmd = 'rm -rf /home/{NAME}/mp4/MP3_LOCAL'
            result = subprocess.check_output(cmd, shell=True)
            self.logger.info("successfully remove local directory")
            cmd = 'mkdir -p /home/{NAME}/mp4/MP3_LOCAL'
            result = subprocess.check_output(cmd, shell=True)
            self.logger.info("successfully create local directory")
        except Exception as e:
            self.logger.error(f"init local/sdfs dir error: {str(e)}")
...
if __name__ == "__main__":
    ...
    file_server.clean_local_sdfs_dir()
    ...
    leader_thread = threading.Thread(target=file_server.leader_main)
    leader_thread.start()

    listening_thread = threading.Thread(target = file_server.reciever)
    listening_thread.start()

    leader_function_thread = threading.Thread(target=file_server.send2Member)
    ...
    while True:
        ...
        try:
            if request_type.lower() == "put_worker":
                local_filename, sdfs_filename = user_input.split(' ')[1], user_input.split(' ')[2]
                file_server.send2Leader(request_type, sdfs_filename, local_filename)
            
            elif request_type.lower() == "put": # host start to leave the gossiping group
                local_filename, sdfs_filename = user_input.split(' ')[1], user_input.split(' ')[2]
                file_server.send2Leader(request_type, sdfs_filename, local_filename)
                ...
                file_server.send2Leader(request_type, sdfs_filename)
                ...
                local_filename = f'/home/{NAME}/mp4/MP3_LOCAL/{sdfs_filename}'
                ...
                for dest_domain_name in random_vms:
                    file_server.send2Leader('get', sdfs_filename, local_filename, dest_domain_name)
                ...
                print(f"Target VMS: {str(random_vms)}")
                for dest_domain_name in random_vms:
                    file_server.send2Leader('put', sdfs_filename, local_filename, dest_domain_name)
            ...
            ################### MP2 COMMANDS ########################
                ...
                leave_status = True 
                file_server.logger.info("leave start")
                for gossiping_func, gossiping_thread in dict(fail_detector.gossiping_threadpool).items():
                    file_server.logger.info(f"joining {str(gossiping_func)}")
                    gossiping_thread.join()
                    file_server.logger.info(f"finished joining {str(gossiping_func)}")
                ...
                file_server.logger.info(f"Finished leaving: {host_domain_name}")
                ...
                file_server.logger.writelog("Joined! Msg_drop_rate {}".format(str(msg_drop_rate)))
                leave_status = False
                ...
                file_server.logger.writelog("Suspicion mode is now {}".format(str(fail_detector.suspicion_mode)))

            elif user_input.lower()[0] == 'r': # host enable message drop
                fail_detector.msg_drop_rate = float(user_input.lower().split(' ')[-1]) / 100
                file_server.logger.writelog("Message rate is now {}".format(str(fail_detector.msg_drop_rate)))

            elif user_input.lower() == 'lm': # list current membership list
                fail_detector.memberlist_lock.acquire()
                file_server.logger.info("Membership List Now: {}".format(str(fail_detector.membership_list)))
           
                fail_detector.memberlist_lock.release()

            elif user_input.lower() == 'm': # sorted version of 'lm'
                # file_server.logger.info("lock debug: {}".format(str(debug_lock)))
                fail_detector.memberlist_lock.acquire()
                file_server.logger.info("Membership List Now: {}".format(str(sorted(membership_list.keys()))))
                fail_detector.memberlist_lock.release()
